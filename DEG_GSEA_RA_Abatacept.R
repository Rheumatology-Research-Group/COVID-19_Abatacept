...
#Model design
attach(sampleTable)
design <- model.matrix(~ week + seq.plate + Sex + age)
detach(sampleTable)

# Transformation to logCPM with voom
# Calculate within sample correlation for blocking
...
saveRDS(res, file=paste0("DEG/RA_Abatacept_DEG.RDS"))
...
saveRDS(edo, file=paste0("GSEA/RA_Abatacept_GSEA.RDS"))
